{- |
 - Este modulo comprende la funcionalidad necesaria para comprimir/descomprimir
   archivos utilizando un algoritmo de codificacion de Huffman.
 - Autor(es): Ricardo Monascal, Gabriela Montoya, Eleazar Leal
 - Version: 1.0
 -}
module CompresorDeArchivos (
	-- ** Operaciones sobre @CompresorDeArchivos@
	leerDeArchivo,
	imprimirArchivo,
	transformar
)
...
{- |
  Dada una direccion de archivo relativa, devuelve el contenidos del
...
{- |
  Dada una direccion de archivo relativa y luego una lista de bytes,
...
{- |
  Transforma un bite en una lista de bits (booleanos).
 -}
byteABits :: Byte -> [Bool]
...
{- |
  Codifica un Arbol de Huffman en una representacion binaria.
  Codificacion:
  Si se lee un True, existen dos posibilidades. Si es el primer True que se lee
  (ya sea al inicio del codigo o después de un False), entonces el siguiente bit
  forma parte del arbol (representado inversamente). De lo contrario, se da por
  finalizado el codigo. Si se lee un False, entonces los siguientes 8 bits,
  corresponden a un simbolo.
 -}
...
{- |
  Transforma una lista de byte en otra, codificandola o decodificandola segun
  sea el caso (True es codificar, false es decodificar).
  Al codificar, el ultimo True se agrega a manera de tener un caracter de fin
  de archivo. (Todos los False hacia el final de la cadena y el ultimo True
  seran luego ignorados por el decodificador).
 -}
